...
type grid = row * row * row  (* DOPOLNI ME *)
...
type result = Winner of player | Tie  (* DOPOLNI ME *)
...
let initial_state : state = OnTurn {player = X; grid = empty_grid}  (* in dodaj anotacijo [: state] *) 

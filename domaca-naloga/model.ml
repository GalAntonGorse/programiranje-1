...
let unrolled_box_fun f (row_ind, col_ind) = function (* POMEMBNO: tukaj naredi memoizacijo *)
  | 0 -> row_ind, col_ind
  | n when n mod 3 = 0 -> f ((row_ind + 1), (col_ind - 2)) (n - 1)
  | n -> f (row_ind, (col_ind + 1)) (n - 1)

let memoize unrolled_box_fun =
  let results = Hashtbl.create 512 in
    let rec mem_f (row_ind, box_ind) n =
      match Hashtbl.find_opt results (row_ind, box_ind, n) with
      | None ->
          let y = unrolled_box_fun mem_f (row_ind, box_ind) n in
          Hashtbl.add results (row_ind, box_ind, n) y;
          y
      | Some y ->
          y
    in
...
let box_fun = memoize unrolled_box_fun
